...
library(ggplot2)
library(chron)
load("C:/Users/super/Desktop/CHIDS/MedStar/UMD CHIDS Data V2.RData")
...
PID_SDID_CATEGORIES_col_name <- c("SDID", "PID", 
                                  "EDUCATIONAL_LEVEL", "DIABETES_FAMILY_HX",
                                  'PHYSICAL_EXERCISE', 'SMOKE_PACK1',
                                  'SMOKE_PACK2', 'SMOKE_YEAR_S', 
                                  'SMOKE_YEAR_Q', 'SMOKE_STATUS',
                                  'SMOKE_ADVICE_QUIT', 'BARRIER_TO_LEARN',
                                  'ALCOHOL_USE', 'MEDICAL_ADHERE',
                                  'DIETARY_COMPLIANCE', 'NEED_FOR_REGULAR_EXERCISE',
                                  'FEELING_DOWN/DEPRESSED/HOPELESS', 'DEPRESSION_SCREENING',
...
SDID_HDID_merge <- function(HDID_list, col_names)
{
  ...
  for (hdid in HDID_list)
  {
    ...
    PID_SDID_CATEGORIES <- merge(PID_SDID_CATEGORIES, 
                                 HD,
                                 by='SDID',
                                 all.x=T
    )
  ...
  # assign col names
  names(PID_SDID_CATEGORIES) <- col_names
  category <- merge(category, PID_SDID_CATEGORIES, by='PID')
  return(category)
}
